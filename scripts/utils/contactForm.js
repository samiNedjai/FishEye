...
function handleKeyboardNavigation(event) {
    if (!modal.classList.contains('modal-visible')) return; // Si le modal n'est pas visible, ne rien faire

    const focusableElements = form.querySelectorAll('input, textarea, button ');
    const firstElement = focusableElements[0];
    const lastElement = focusableElements[focusableElements.length - 1];

    // Vérifier si le focus est dans le formulaire
    const isFocusInForm = Array.from(focusableElements).some(element => element === document.activeElement);

    if (!isFocusInForm) {
        firstElement.focus(); // Si le focus n'est pas dans le formulaire, déplacer le focus au premier élément du formulaire
    }
...
// Attacher les gestionnaires d'événements pour la navigation au clavier
